...
for v in V:
    m.addConstr(quicksum(d[j] * quicksum(z[j, k, v] for k in K[j]) for j in N) <= C)
...
plt.title('Vehicle Routing Problem Solution Case 1')
...
plt.legend()
plt.savefig('../Figs/question_e_case1.png')
plt.show()
